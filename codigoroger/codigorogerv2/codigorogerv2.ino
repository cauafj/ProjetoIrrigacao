...
char timestamp[20]; // variável para conter o timestamp
const int sensorPin = A0; //entrada do sensor
const int valvPin = 8; //entrada do relé
const float DRY = 23; //limiar de umidade - seco
const float ENCHARCADO = 27; //limiar de umidade - molhado
...
void loop() {
  ...
  //se a umidade for menor ou igual o limiar, liga o motor, a irrigação inicia
  if(moisture <= DRY) {
    digitalClockDisplay();
    cicloirrigacao++;
    dripping = TRUE;
  ...
  //se a umidade for maior ou igual o limiar, desliga o motor, a irrigação para
  } else if (moisture >= ENCHARCADO) {
      digitalClockDisplay();
      digitalWrite(valvPin, HIGH);
